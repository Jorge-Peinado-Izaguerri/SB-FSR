...
# in QC_criteria file AND where the number of timepoints after t0 above this threshold are fewer than 4
tn_M1_M2_low_intensity<-skyline_output %>%
  mutate(x1.valid=case_when(X1.Area>M1_M2_min~1,TRUE~0),
         ...
         x4.valid=case_when(X4.Area>M1_M2_min~1,TRUE~0)) %>%
  mutate(total.valid=x1.valid+x2.valid+x3.valid+x4.valid) %>%
  filter(Fragment.Ion %in% c("precursor [M+1]","precursor [M+2]","precursor [M+3]") & 
           total.valid < 4 &
           (X1.Area<M1_M2_min |
              X2.Area<M1_M2_min |
...
tn_low_minM0<-skyline_output %>%
  mutate(x1.valid=case_when(X1.Area>minM0~1,TRUE~0),
         ...
         x4.valid=case_when(X4.Area>minM0~1,TRUE~0)) %>%
  mutate(total.valid=x1.valid+x2.valid+x3.valid+x4.valid) %>%
...
# Now remove these results from the skyline_output file. Removes whole time series (see note above about removing
# individaul timepoints later).
...
RT_filter<-skyline_output %>%
  select(23,6,9,12,14,16,18,20,22) %>%
  gather(day_long,RT,5:9) %>% 
...
# configured for 5 timepoints over one phase and M+3
phase1<-skyline_output %>%
  select(Subject,Protein.Gene,Protein.Accession,Protein.Name,Protein.Description,Peptide.Modified.Sequence.Three.Letter.Codes,Deut_labs,Fragment.Ion,Product.Charge,Transition.Count,X0.Area,X1.Area,X2.Area,X3.Area,X4.Area) %>%
  gather(day_long,Area,11:15) %>%
...
FSR<-phase1 %>%
#  filter(Peptide.Sequence %in% c("A[1Ac]DQAISFLK","AIYQFTSESPDVIFR")) %>%
...
#                 mutate(Diff=(Fit-mpe)^2/mpe) %>%   # Run either this line of code or the line above but NOT both lines
